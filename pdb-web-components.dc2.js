...
/* harmony default export */ __webpack_exports__["a"] = (PdbDataLoader);
...
const categoryUrls = new Map([['PrimaryCitation', {
  url: 'citations',
  responseData(data) {
  ...
  title(responseData) {
    switch (responseData) {
      ...
      case 'unpublished':
        return 'The PDB does not have any information about ' + 'a published paper describing this entry';
      default:
        return 'This icon conveys information about whether or not a paper ' + 'describing this entry has been published';
    }
  }
}], ['Taxonomy', {
  url: 'analysis',
  ...
  title(responseData) {
    switch (responseData) {
      case 'artificial':
        return 'The biomacromolecules in this entry were artificially designed';
      case 'other':
        return 'Information about the source organism of the biomacromolecules' + ' in this entry can be obtained by clicking on the icon';
      case 'multiple':
        return 'The biomacromolecules in this entry derive ' + 'from multiple organisms';
      case 'fly':
        return 'The source organism of the biomacromolecules ' + 'in this entry is fruit-fly';
      case 'CGIlogoUnknown':
        return 'This icon conveys information about the source organism ' + 'of the biomacromolecules in this entry';
      default:
        return 'The source organism of the biomacromolecules in this entry is ' + responseData;
    }
  }
}], ['Expressed', {
  url: 'analysis',
  ...
  title(responseData) {
    switch (responseData) {
      case 'expressed':
        return 'The sample of the biomacromolecules in this entry was ' + 'obtained by expression and purification';
      case 'synthetic':
        return 'The sample of the biomacromolecules in this entry was ' + 'obtained by chemical synthesis';
      case 'purified':
        return 'The sample of the biomacromolecules in this entry was ' + 'extracted and purified from a biological source';
      case 'multiple':
        return 'The sample of the biomacromolecules in this entry was ' + 'obtained using multiple techniques';
      case 'other':
        return 'The sample of the biomacromolecules in this entry was ' + 'obtained using techniques other than expression, ' + 'chemical synthesis or extraction from a biological source';
      case 'CGIlogoUnknown':
        return 'This icon conveys information about the production techniques ' + 'used to obtain the biomacromolecular samples for this entry';
      default:
  ...
  title(responseData) {
    ...
    switch (responseData.replace('NoData', '')) {
      case 'xray':
        output = 'The structure was determined using X-ray crystallography';
        break;
      case 'nmr':
      ...
      case 'em':
        ...
        break;
      case 'multiple':
        output = 'The structure was determined using Electron Crystallography';
        break;
      case 'extal':
        output = 'The sample of the biomacromolecules in this entry was ' + 'obtained using techniques other than expression, ' + 'chemical synthesis or extraction from a biological source';
        break;
      case 'hybrid':
        output = 'The structure was determined using a hybrid technique';
        break;
      case 'other':
        output = 'The structure was determined using a technique other than ' + 'X-ray, NMR, EM, or a hybrid thereof';
        break;
      default:
        return 'This icon conveys information about the ' + 'structure-determination technique for this entry';
    }
    return `${output} (${responseData.endsWith('NoData') ? 'no ' : ''}experimental data available)`;
  }
}], ['Protein', {
  url: 'analysis',
  responseData(data) {
  ...
  title(responseData) {
    switch (responseData) {
      ...
      case 'CGIlogoUnknown':
        return 'This icon conveys information about the presence ' + 'of protein in the entry';
      default:
  ...
  title(responseData) {
    switch (responseData) {
      ...
      case 'CGIlogoUnknown':
        return 'This icon conveys information about the presence ' + 'of nucleic acid (DNA, RNA) in the entry';
      default:
        throw new Error('Something bas happened');
    }
  }
  ...
  title(responseData) {
    switch (responseData) {
      ...
      case 'CGIlogoUnknown':
        return 'This icon conveys information about the presence ' + 'of ligands in the entry';
      default:
        throw new Error('Something bas happened');
    }
  }
...
class PdbPrints extends HTMLElement {
  ...
  _render() {
    ...
    const style = `
      <style>
        ...
        :host > .root {
          ...
          display: -ms-flexbox;
          display: flex;
          ${this.orientation === 'horizontal' ? `-webkit-box-orient: horizontal;
    ...
    (this.shadyRoot || this.shadowRoot).innerHTML = `
      ...
      <span class="root">
        ...
        ${[...categoryUrls.entries()].filter(([c]) => this._include.has(c) && !this._exclude.has(c)).map(([category, { url, responseData, title }]) => {
      ...
      return `
                ...
                >
                  <img
                    src="//www.ebi.ac.uk/pdbe/widgets/pdblogos/transparent/${this.size}/${category}_${_responseData}.png"
                    alt="${_title}"
                  >
                </a>
              `.trim();
  ...
  set size(value) {
    ...
    if (!supportedSizes.has(_value)) {
      _value = [...supportedSizes].sort((a, b) => Math.abs(a - _value) - Math.abs(b - _value))[0];
      console.warn(`${value} is not a supported value, setting to closest one: ${_value}`);
    }
    this._size = _value;
...
/* harmony default export */ __webpack_exports__["a"] = (PdbPrints);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__pdb_data_loader__ = __webpack_require__(29);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__pdb_prints__ = __webpack_require__(30);



const PdbDataLoader = __WEBPACK_IMPORTED_MODULE_0__pdb_data_loader__["a" /* default */];
/* harmony export (immutable) */ __webpack_exports__["PdbDataLoader"] = PdbDataLoader;

const PdbPrints = __WEBPACK_IMPORTED_MODULE_1__pdb_prints__["a" /* default */];
/* harmony export (immutable) */ __webpack_exports__["PdbPrints"] = PdbPrints;


/***/ })

});
//# sourceMappingURL=pdb-web-components.dc2.js.map